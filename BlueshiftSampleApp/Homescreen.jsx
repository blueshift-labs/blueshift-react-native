...
export default function HomeScreen({navigation}) {
  useEffect(() => {
    ...
    //start
    // Initialise Blueshift
    Blueshift.init();

    // Get the deep link when app launched from killed state
    Linking.getInitialURL().then(url => {
      // iOS only
      // This url parameter will either be a deeplink to open the app (links.app.eonnext.com)
      // OR, a the ORIGINAL url which had been coverted to a deeplink via BlueShift comms
    ...
    // Add event listner for deeplink `url` event
    global.urlListener = Linking.addEventListener("url", event => {
      // iOS only
      // This url parameter will either be a deeplink to open the app (links.app.eonnext.com)
      // OR, a the ORIGINAL url which had been coverted to a deeplink via BlueShift comms
      ...
      if (url) {
        ...
        if (hasBlueshiftPath) {
          // ANDROID ONLY:
          // Will detect the original url from the deeplink
          // If link is NOT an app link, open url in browser
          Blueshift.processBlueshiftUrl(url);
        }
